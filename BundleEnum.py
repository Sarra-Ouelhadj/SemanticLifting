...
class BundleEnum(Bundle):
    ...
    def validate(self):
        ...
        for enum_val in self.values:
            ...
            #1. Chaque valeur d'énumération doit avoir un lien de référence ou une définition
            if ((enum_val['IRI']=='' or enum_val['IRI']== None) and (enum_val['definition']=='' or enum_val['definition'] == None )): errors.append (f"La valeur d'énumération `{enum_val['name']}` de l'énumération `{self.name}` doit avoir un lien de référence ou une définition")
    ...
    def generateOntology(self, vocabulary_path:str):
        ...
        if (nb_value_created > 0):
            print(f"Le nombre de valeurs créées pour l'énumération `{self.name}` est : {nb_value_created}")
