...
def simplify_bundle_for_demo(bundle : Bundle, to_keep: list=['reseau_loc', 'revet_d', 'sens_d']):
    """
    ...
    # MAJ du dataset
    rest_of_enums = [lkd['destination'].source for lkd in bundle.linked_to if (lkd['destination'].name not in to_keep and type(lkd['destination']) == BundleEnum)]
    bundle.dataset.drop(columns=rest_of_enums, inplace=True)
    
    BundleCollection(bundle) # classe statique à modifier pour permettre d'avoir + instances de BundleCollection en cas de manipulation de + JDD en même temps
    return bundle
...
# df = gpd.read_file(dataset, rows = 100) # chargement d'une partie du fichier
